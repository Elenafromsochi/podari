...
export function WishDetails({ wishId, onBack, onFulfilled, onDeleted }: Props) {
  ...
  // Карточка желания — оверлей поверх ленты (как GiftDetailModal), а не
  // отдельная страница: Esc закрывает, как и клик по фону/крестику.
  useEffect(() => {
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[110] flex items-start justify-center overflow-y-auto bg-black/60 p-3 animate-fade-in sm:items-center sm:p-6"
      onClick={onBack}
    >
      <div className="relative my-auto w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="relative max-h-[92dvh] overflow-y-auto rounded-3xl border bg-card shadow-xl">
          <button
            type="button"
            onClick={onBack}
            aria-label="Закрыть"
