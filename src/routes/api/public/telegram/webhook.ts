...
async function sendLoginConfirmed(chatId: number) {
  ...
  // со своей защищённой initData для мгновенного входа) — на практике
  // оказалось ненадёжно: initData не появлялась (пустой location.hash) на
  // части устройств/клиентов, человек просто попадал на обычный экран
  // логина без объяснений. Обычная url-кнопка открывает сайт как всегда —
  // проще, без встроенного WebView, вход — тем же способом (Telegram/VK/
